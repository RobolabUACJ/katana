	...
	for(hTCP = 0; hTCP < TCP_SOCKET_COUNT; hTCP++)
	{
		...
		if(MyTCBStub.smState == TCP_CLOSE_WAIT)
		{
			...
			if((SHORT)(MyTCBStub.OverlappedTimers.closeWaitTime - (WORD)TickGetDiv256()) <= (SHORT)0)
			{
				vFlags = FIN | ACK;
				MyTCBStub.smState = TCP_LAST_ACK;
                                //Desconexion TCP Automatica. Cuando el socket TCP del driver se desconecta, envia un
                                //paquete FIN para terminar la conexion. Esta rutina se realiza en esta funcion de la
                                //libreria TCP. Apaga los LEDS indicando la desconexion del Cliente. Si alguno de los
                                //Clientes sigue conectado, el LED indicado encendera en el ciclo del main().
                                LED1_IO=0;
                                LED2_IO=0;
                                LED3_IO=0;
			}
